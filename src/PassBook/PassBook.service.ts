...
export class PassBookService{
    ...
    private cacheservice:ClearCache,
    ){}
    ...
    async saveSavingsdeposit(passbookdto:PassBookDTO,user:User):Promise<IReponse<PassBook>>{
        ...
        try{
            ...
            this.cacheservice.clearCache(CachKeyPassbook.GET_PASSBOOK_CACHE_KEY_TOTAL_PASSBOOK);
    ...
    async getTotalCycles(svdid):Promise<any>{
        if(CachKeyPassbook.GET_PASSBOOK_CACHE_KEY_TOTAL_PROFIT==""){
            CachKeyPassbook.GET_PASSBOOK_CACHE_KEY_TOTAL_PROFIT=svdid.toString()+"PROFIT";
        }
    ...
    async GetAllPassbookByUserId(user:User):Promise<any>{
        ...
        if(CachKeyPassbook.GET_PASSBOOK_CACHE_KEY_TOTAL_PASSBOOK==""){
            CachKeyPassbook.GET_PASSBOOK_CACHE_KEY_TOTAL_PASSBOOK=user._id.toString()+"GETALLPASSBOOK";
        }
        return passbook;
    }

    async GetPassbookIsActive(user:User):Promise<any>{
        if(CachKeyPassbook.GET_PASSBOOK_CACHE_KEY_IS_ACTIVE==""){
            CachKeyPassbook.GET_PASSBOOK_CACHE_KEY_IS_ACTIVE=user._id.toString()+"GET_PASSBOOK_CACHE_KEY_IS_ACTIVE";
        }
        const passbook=await this.passbookmodel.find({userId:user._id,status:false});
        return passbook;
    }

    async GetPassBookById(id,user:User):Promise<PassBook>{
